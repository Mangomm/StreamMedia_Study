...
bool MainWid::Init()
{
    ...
    if(mp_->Init() < 0)
    {
        LOG_ERROR("IjkPlayerQt init faileld");
        return false;
    }
...
bool MainWid::ConnectSignalSlots()
{
    ...
    // ctrlbar - ijkplayer_qt
    connect(ui->CtrlBarWid, &CtrlBar::SigPlayOrPause, mp_, &IjkPlayerQt::OnIjkMediaPlayer_prepareAsync);
    connect(ui->CtrlBarWid, &CtrlBar::SigStop, mp_, &IjkPlayerQt::OnIjkMediaPlayer_stop);
    return true;
}
